...
Host gateway
    HostName kdesvr1.ccs.tsukuba.ac.jp        # ← ゲートウェイのアドレス
    User <UserName>
    IdentityFile <path for identity file such as "C:\Users\horie\.ssh\id_ed25519">

Host workstation
    HostName 192.168.0.124            # ← ワークステーションのIP
    User <UserName>
    ProxyJump gateway
...
Github User Setting PageのSSH and GPG keys → New SSH Key → ペースト 
...
'''
→ Githubにはつながっていますけど，Bashからのログインを想定していません！みたいなメッセージが出たら設定正しくできています．
...
3. setup-git-and-ssh.shを再実行（Docker環境におけるGithub設定を確立します．）
4. python hogehoge.py（pythonコードを実行します）
5. "hello world"が表示されたらOKです．
